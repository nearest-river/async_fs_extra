  ...
  Q: AsRef<Path>,
{
  ...
  if !options.overwrite && exists(&to).await {
  ...
  F: FnMut(TransitProcess),
{
  ...
  let mut file_from = File::open(from).await?;
  let mut buf = vec![0; options.buffer_size];
  let file_size = file_from.metadata().await?.len();
  let mut copied_bytes: u64 = 0;
  let mut file_to = File::create(to).await?;
  while !buf.is_empty() {
    match file_from.read(&mut buf).await {
      Ok(0) => break,
      Ok(n) => {
        let written_bytes = file_to.write(&buf[..n]).await?;
        if written_bytes != n {
...
///
/// ```
pub async fn move_file<P, Q>(from: P, to: Q, options: &CopyOptions) -> Result<u64>
where
  P: AsRef<Path>,
  Q: AsRef<Path>,
{
  ...
  let result = copy(&from, to, options).await?;
  if is_remove {
    remove(from).await?;
  }
...
///
/// ```
pub async fn move_file_with_progress<P, Q, F>(
  from: P,
  to: Q,
  options: &CopyOptions,
  progress_handler: F,
  ...
  F: FnMut(TransitProcess),
{
  ...
  let result = copy_with_progress(&from, to, options, progress_handler).await?;
  if is_remove {
    remove(from).await?;
  }
...
///
/// ```
pub async fn remove<P>(path: P) -> Result<()>
where
  P: AsRef<Path>,
{
  ...
  P: AsRef<Path>,
{
  ...
  Ok(f.write_all(content.as_bytes()).await?)
}
