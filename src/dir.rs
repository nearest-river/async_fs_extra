...
/// ```
pub async fn get_details_entry<P>(
  path: P,
  ...
  P: AsRef<Path>,
{
  ...
  get_details_entry_with_meta(path, config, metadata).await
}

async fn get_details_entry_with_meta<P>(
  path: P,
  config: &HashSet<DirEntryAttr>,
  metadata: Metadata,
) -> Result<HashMap<DirEntryAttr, DirEntryValue>>
      ...
      Err(_) => {
        if let Some(parent_path) = path.parent() {
          if let Some(name) = path.file_name() {
            result_path = fs::canonicalize(&parent_path).await?;
...
/// ```
pub async fn ls<P>(path: P, config: &HashSet<DirEntryAttr>) -> Result<LsResult>
where
  P: AsRef<Path>,
{
  ...
  while let Some(entry)=stream.next_entry().await? {
    ...
    let item=get_details_entry_with_meta(path,&config,metadata).await?;
  ...
  if config.contains(&DirEntryAttr::BaseInfo) {
    base = get_details_entry(&path, &config).await?;
  }
...
/// ```
pub async fn create<P>(path: P, erase: bool) -> Result<()>
where
  P: AsRef<Path>,
{
...
pub async fn create_all<P>(path: P, erase: bool) -> Result<()>
where
  P: AsRef<Path>,
{
  if erase && fs::try_exists(&path).await? {
...
///
/// ```
pub async fn copy<P, Q>(from: P, to: Q, options: &CopyOptions) -> Result<u64>
where
  P: AsRef<Path>,
  Q: AsRef<Path>,
{
  ...
  if (exists(&from).await || !options.copy_inside) && !options.content_only {
    to.push(dir_name);
  }
  ...
  let dir_content = get_dir_content2(from, &read_options).await?;
  for directory in dir_content.directories {
    let tmp_to = Path::new(&directory).strip_prefix(from)?;
    let dir = to.join(&tmp_to);
    if !dir.exists() {
      if options.copy_inside {
        create_all(dir, false).await?;
      } else {
        create(dir, false).await?;
      }
    }
  }
  ...
  for file in dir_content.files {
    ...
    while work {
      result_copy = super::file::copy(&file, &path, &file_options).await;
      match result_copy {
        Ok(val) => {
          result += val;
          work = false;
...
/// ```
///
pub async fn get_dir_content<P>(path: P) -> Result<DirContent>
where
  P: AsRef<Path>,
{
  let options = DirOptions::new();
  get_dir_content2(path, &options).await
}
...
/// ```
///
pub async fn get_dir_content2<P>(path: P, options: &DirOptions) -> Result<DirContent>
where
  P: AsRef<Path>,
{
  let mut depth = 0;
  ...
  _get_dir_content(path, depth).await
}

async fn _get_dir_content<P>(path: P, mut depth: u64) -> Result<DirContent>
where
  P: AsRef<Path>,
{
  ...
  if is_dir(&path).await {
    ...
    if depth == 0 || depth > 1 {
      ...
      while let Some(entry)=stream.next_entry().await? {
        ...
        match _get_dir_content(_path, depth).await {
          Ok(items) => {
    ...
    dir_size = fs::metadata(&path).await?.len();
    files.push(item);
  }
  Ok(DirContent {
    dir_size,
...
/// ```
pub async fn get_size<P>(path: P) -> Result<u64>
where
  P: AsRef<Path>,
{
...
///
/// ```
pub async fn copy_with_progress<P, Q, F>(
  from: P,
  to: Q,
  options: &CopyOptions,
  ...
  F: FnMut(TransitProcess) -> TransitProcessResult,
{
  ...
  if (exists(&to).await || !options.copy_inside) && !options.content_only {
    to.push(dir_name);
  }
  ...
  for file in dir_content.files {
    ...
    info_process.file_total_bytes = fs::metadata(&file).await?.len();
    ...
    while work {
      {
        ...
        result_copy =
          super::file::copy_with_progress(&file, &path, &file_options, _progress_handler).await;
      }
      match result_copy {
        Ok(val) => {
          result += val;
...
///
/// ```
pub async fn move_dir<P, Q>(from: P, to: Q, options: &CopyOptions) -> Result<u64>
where
  P: AsRef<Path>,
  Q: AsRef<Path>,
{
  let mut is_remove = true;
  if options.skip_exist && exists(to.as_ref()).await && !options.overwrite {
    is_remove = false;
  }
  let from = from.as_ref();
  ...
  let dir_content = get_dir_content(from).await?;
  for directory in dir_content.directories {
    let tmp_to = Path::new(&directory).strip_prefix(from)?;
    let dir = to.join(&tmp_to);
    if !exists(&dir).await {
...
///
/// ```
pub async fn move_dir_with_progress<P, Q, F>(
  from: P,
  to: Q,
  options: &CopyOptions,
  mut progress_handler: F,
...
{
  let mut is_remove = true;
  if options.skip_exist && exists(&to).await && !options.overwrite {
    is_remove = false;
  }
  let from = from.as_ref();
